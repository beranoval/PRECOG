...
def add_target_opencitatins_marginal(target_year,df,target_col_name,get_cum=False): 
    ...
    df_joined[target_col_name] =  np.where( df_with_med["OpenCitations"]> df_with_med["median_cit"],1, 0)
    print(df_joined[target_col_name].value_counts())
    print(len(df_joined))
...
def x_first_last_val(results,perc):
    ...
    vysledky_des = results[results["OpenCitations"]<=min(results["OpenCitations"])].sample(len(vysledky_asc))
